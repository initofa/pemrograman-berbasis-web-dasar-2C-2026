...
<head>
    ...
    <title>Tambah Buku - Perpustakaan SDN Dumajah 4</title>
...
<body class="bg-gray-100">
    ...
    <div class="container mx-auto px-4 py-8">
        <div class="flex flex-col md:flex-row gap-6">
            <div class="md:w-64 space-y-2">
                ...
                <a href="buku_tambah.php" class="block px-4 py-2 bg-red-600 text-white rounded-lg transition">
            ...
            <div class="flex-1">
                <div class="max-w-2xl mx-auto bg-white rounded-lg shadow p-8">
                    <div class="flex items-center mb-6">
                        <i class="fas fa-plus-circle text-red-600 text-3xl mr-3"></i>
                        <h1 class="text-2xl font-bold text-red-700">Tambah Buku Baru</h1>
                    </div>
                    ...
                    <form method="POST" id="tambahForm">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="penulis" required class="w-full px-3 py-2 border rounded">
                        </div>
                        
                        <div class="mb-4">
                            <label class="block font-semibold mb-1"><i class="fas fa-building text-red-500 mr-1"></i> Penerbit *</label>
                            <input type="text" name="penerbit" required class="w-full px-3 py-2 border rounded">
                        </div>
                        
                        <div class="grid grid-cols-2 gap-4 mb-4">
                            <div>
                                <label class="block font-semibold mb-1"><i class="fas fa-calendar text-red-500 mr-1"></i> Tahun Terbit</label>
                                <input type="number" name="tahun" id="tahun" required class="w-full px-3 py-2 border rounded">
                            </div>
                            <div>
                                <label class="block font-semibold mb-1"><i class="fas fa-boxes text-red-500 mr-1"></i> Stok</label>
                                <input type="number" name="stok" id="stok" value="1" class="w-full px-3 py-2 border rounded">
                            </div>
                        </div>
                        
                        <div class="mb-6">
                            <label class="block font-semibold mb-1"><i class="fas fa-tag text-red-500 mr-1"></i> Harga (Rp)</label>
                            <input type="number" name="harga" id="harga" step="1000" class="w-full px-3 py-2 border rounded">
                        </div>
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (empty($judul) || empty($penulis) || empty($penerbit)) {
            echo "<script>Swal.fire({icon:'error',title:'Gagal!',text:'Semua field harus diisi!',confirmButtonColor:'#dc2626'});</script>";
        } elseif ($tahun < 1900 || $tahun > date('Y')) {
            echo "<script>Swal.fire({icon:'error',title:'Gagal!',text:'Tahun tidak valid!',confirmButtonColor:'#dc2626'});</script>";
        } else {
            $stmt = $conn->prepare("INSERT INTO buku (judul, penulis, penerbit, tahun_terbit, stok, harga) VALUES (?, ?, ?, ?, ?, ?)");
            $stmt->bind_param("sssiii", $judul, $penulis, $penerbit, $tahun, $stok, $harga);
            if ($stmt->execute()) {
                echo "<script>
                    Swal.fire({icon:'success',title:'Berhasil!',text:'Buku berhasil ditambahkan!',confirmButtonColor:'#dc2626'})
                    .then(()=>{window.location.href='buku.php';});
                </script>";
            } else {
                echo "<script>Swal.fire({icon:'error',title:'Gagal!',text:'Gagal menambahkan buku!',confirmButtonColor:'#dc2626'});</script>";
            }
            $stmt->close();
        }
    }
    ...
    <script>
        document.getElementById('tambahForm')?.addEventListener('submit', function(e) {
            ...
            if (tahun < 1900 || tahun > tahunNow) {
                e.preventDefault();
                Swal.fire({icon:'error',title:'Gagal!',text:'Tahun terbit harus antara 1900 - ' + tahunNow,confirmButtonColor:'#dc2626'});
                return false;
            }
            return true;
        });
